...
def select_random_accounts
random_account = []
number_of_unique_random_account = 0 

    while number_of_unique_random_account < 5 do
        random_account << list_test.sample #Select ramdom accounts in the list
        unique_random_account = random_account.uniq #Delete duplicates
        number_of_unique_random_account = unique_random_account.length #number of elements in the new array
    end
    print unique_random_account

    unique_random_account.each {
        |username| client.update("#{username} voici un envoie de tweet automatisé et développé en Ruby !!!! #bonjour_monde")
        puts "tweet envoyé à #{username}" }
end

def perform
    login_twitter
    select_random_accounts

def tweet (x)
    
    x.each do |username| #boucle qui réitere pour chaque cible auquel on envoi un tweet
    client.update(username + ' Je suis étudiant à The Hacking Projet, une formation gratuite pour apprendre à coder et faire des bots')
    puts "tweet envoyé à #{username}, attendre 30 secondes"
    sleep(70) #fonction qui attend 70 secondes avant de continuer a tourner le bot
    end
=begin
    i=0
    while i < 5 do
    client.update("Hello #{x[i]} voici un envoie de tweet automatisé et développé en Ruby !!!! #bonjour_monde")}
    i+=1
    end
